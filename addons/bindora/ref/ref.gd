...
const TYPE_CONVERT_RULES: Dictionary = {
...
func _set_value(_new_value) -> void:
	...
	if not Engine.is_editor_hint():
		if __type__ == TYPE_NIL:
			...
			push_error("Type error, value should be %s" % type_string(__type__))
			return
...
func _type_convert_check(_new_type: int) -> bool:
	if __type__ in TYPE_CONVERT_RULES:
		return _new_type in TYPE_CONVERT_RULES[__type__]
	return false
